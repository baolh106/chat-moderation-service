...
from modules.key.infrastructure.mongo.service_key_repository import ServiceKeyRepository
from modules.key.use_case.verify_api_key import VerifyApiKeyUseCase
...
async def validate_api_key(
    ...
    if not key_record:
        logger.warning(f"Xác thực thất bại cho API Key: {api_key}")
        raise HTTPException(
